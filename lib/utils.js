...
exports.matchRoutes = function (req, res) {
  ...
  // var temp_data = req.session.data || {} ;
  req.session.data = req.session.data || {}
  req.data = req.data || {}
  var data = merge(req.data,req.session.data);

  res.render(path, {"data":data}, function (err, html) {
    if (err) {
      res.render(path + '/index', {"data":data}, function (err2, html) {
        if (err2) {
